...
COMMON_DEFCONFIG=vendor/xiaomi/mi8996_defconfig
...
if [ "$DEVICE" = "H850" ]; then
  ...
  DEVICE_DEFCONFIG=vendor/xiaomi/gemini.config
  BOARD_DEFCONFIG=vendor/xiaomi/gemini.config
...
SETUP_BUILD() {
    ...
    if [ $SINGLEBUILD = "yes" ]; then
	    make -C "$RDIR" O=$BDIR CROSS_COMPILE=$CROSS_COMPILE $COMMON_DEFCONFIG $DEVICE_DEFCONFIG \
